...
export function VerticalCalendar({
  ...
  useEffect(() => {
    ...
    if (selectedTemplate) {
      ...
      if (template) {
        filtered = filtered.map(day => ({
          ...day,
          shifts: day.shifts.filter(shift => shift.startTime === template.startTime && shift.endTime === template.endTime)
  ...
  return <div className="space-y-2">
      <div className="flex items-center justify-between">
        {isAdmin() && <Sheet open={isFilterOpen} onOpenChange={setIsFilterOpen}>
            <SheetTrigger asChild>
              <Button variant="outline" size="icon" className="relative">
                <Filter className="h-4 w-4" />
                {selectedTemplate && <span className="absolute -top-1 -right-1 h-2 w-2 rounded-full bg-primary" />}
              </Button>
            </SheetTrigger>
            <SheetContent>
              <SheetHeader>
              ...
              <div className="py-6 space-y-6">
                ...
                <Button variant="outline" onClick={clearFilters} className="w-full" disabled={!selectedTemplate}>
                  Cancella filtri
                </Button>
              </div>
            </SheetContent>
      ...
      {isLoading ? <div className="flex justify-center py-12">
        ...
        </div> : currentMonthDays.length === 0 ? <div className="bg-muted/20 p-6 rounded-md text-center">
          <p className="text-muted-foreground">
            {showOnlyUserShifts || selectedTemplate ? "Nessun turno corrisponde ai filtri selezionati" : "Nessun turno programmato per questo mese"}
          </p>
          {selectedTemplate && <Button variant="link" onClick={clearFilters} className="mt-2">
              Cancella filtri
            </Button>}
        </div> : <div className="space-y-3 animate-in fade-in duration-300" ref={listRef}>
          {currentMonthDays.map(day => <div key={day.date.toISOString()} data-today={day.isToday} className={cn("py-2 px-1", day.isToday ? "bg-primary/5 rounded-md" : "")}>
              <div className="flex justify-between items-center mb-3">
                <div className="flex items-center gap-2">
                  <Badge variant={day.isToday ? "default" : "outline"} className={cn("text-xs font-normal capitalize", day.isToday ? "" : "bg-muted/20")}>
                    <CalendarIcon className="h-3 w-3 mr-1" />
                    {day.dayName} {day.dayNumber}
                  </Badge>
                </div>
                {isAdmin() && <Button variant="ghost" size="sm" className="h-7 px-2 text-xs" onClick={() => onAddShift(day.date, day.dayOfWeek)}>
                    + Aggiungi
                  </Button>}
              </div>
              
              {day.shifts.length > 0 ? <div className="space-y-1.5 pl-1">
                  {day.shifts.map(shift => {
            const employee = getEmployeeById(shift.employeeId);
            if (!employee) return null;
            return <ShiftItem key={shift.id} shift={shift} employee={employee} onClick={() => onEditShift(shift)} highlight={shouldHighlightShift(shift)} isFilterActive={showOnlyUserShifts} />;
          })}
                </div> : day.isFreeDay ? <div className="px-1 animate-in slide-in-from-top-2 duration-300">
                  <FreeDayIndicator date={day.date} />
                </div> : <p className="text-sm text-muted-foreground pl-1 py-1">Nessun turno programmato</p>}
